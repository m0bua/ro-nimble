...
class IndexGoodsGroupsConstructors extends ExtCommand
{
    /**
     * @var string
     */
    ...
    {
        $constructorsGroupsQuery = DB::connection('nimble_read')
            ->table('promotion_groups_constructors as pgc')
            ->select([
                'pc.id',
                'pc.promotion_id',
                'pc.gift_id',
        ...
        ChunkCursor::iterate($constructorsGroupsQuery, function ($constructors) use (&$promotionGroupsConstructorsIDs) {
            $constructorsData = [];
